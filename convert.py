...
import argparse
import warnings
from generator import generate_m3u_playlist

deprecation_message = "这个脚本已被弃用，并可能在未来的版本中移除。请使用 'generator.py' 代替。This script is deprecated, and may be removed in future versions. Please use 'generator.py' instead."
...
parser.add_argument("--epg-url", default=DEFAULT_EPG, help=f"eg: {DEFAULT_EPG}")
parser.add_argument("--logo-url", default=DEFAULT_LOGO, help=f"eg: {DEFAULT_LOGO}")
parser.add_argument("--output", default=os.path.join(DIR_SCRIPT, "iptv.m3u"), help="保存路径，默认为脚本目录的 iptv.m3u")
args = parser.parse_args()

# Old ways of replacing URLs
...
m3u_file = generate_m3u_playlist(
    json_path_list=["playlist-zz.json"], 
    key_live=["bjunicom-multicast-httpproxy"],
    key_timeshift=[],
    rtp_proxy_url=args.rtp_url,
